    ...
    try{
		...
		if(minPrice && maxPrice){
			...
			if(isNaN(parsedMin) || parsedMin < 0 || isNaN(parsedMax) || parsedMax < 0){
				inputErrors.push("minPrice or maxPrice not a valid number.");
			}else{
				conditions.push('L_SystemPrice BETWEEN ? AND ?');
				queryVals.push(parsedMin);
				queryVals.push(parsedMax);
			}
		}else{
			if(minPrice){
...
router.post('/batch', async (req, res) => {
	try{
		...
		const query = `SELECT * FROM rets_property WHERE id IN (${placeholders}) OR L_ListingID IN (${placeholders})`;
		
		const queryParams = [...cleanIds, ...cleanIds];
...
router.get('/:id/openhouses', async (req, res) => {
    ...
    try{
        ...
        //get the property
        const [property] = await pool.query('SELECT L_ListingID FROM rets_property WHERE id = ? OR L_ListingID = ?', [propertyId, propertyId]);
        ...
        //return the openhouses if there are any otherwise the array will be empty
...
router.get('/:id', async (req, res) => {
    try{
        ...
        const [property] = await pool.query('SELECT * FROM rets_property WHERE id = ? OR L_ListingID = ?', [propertyId, propertyId]);
